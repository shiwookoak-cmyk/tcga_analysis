# TCGA Analysis Pipeline

Scripts for downloading TCGA data, running gene-wise survival analysis,
performing pathway enrichment analysis, visualizing KEGG metabolic pathways,
calculating ssGSEA immune signature scores, comparing mRNA expression, and
creating oncoprints.

Downloaded TCGA data and generated outputs are saved under `data/` and are not
intended to be uploaded to GitHub.

## Repository Structure

```text
tcga_analysis/
|-- src/
|   |-- 01_download_tcga.R
|   |-- 02_gene_survival_analysis.R
|   |-- 03_pathway_analysis.R
|   |-- 04_map_kegg_pathways.R
|   |-- 05_run_ssgsea.R
|   |-- 06_analyze_expression.py
|   `-- 07_oncoprint.py
|-- renv/
|   |-- activate.R
|   `-- settings.json
|-- .Rprofile
|-- run_luad_pipeline.sh
|-- figures/
|   |-- LUAD_coxph_continuous_gsea_kegg_map.svg
|   |-- LUAD_ssgsea_signature_correlation_heatmap.svg
|   |-- LUAD_ssgsea_sample_score_heatmap.svg
|   |-- LUAD_PLA2G4A_paired_tumor_normal_box.svg
|   |-- LUAD_PLA2G4A_STK11_mutation_group_box.svg
|   |-- LUAD_PLA2G4A_PTGS2_scatter_sample_type.svg
|   |-- LUAD_PLA2G4A_PTGS2_STK11_scatter.svg
|   |-- LUAD_STK11_gene_group_heatmap.svg
|   |-- LUAD_oncoprint_fisher_heatmap.svg
|   `-- LUAD_oncoprint.svg
|-- immune_signature.gmt
|-- renv.lock
|-- requirements.txt
`-- README.md
```

## Requirements

### R Packages

Exact R package versions are recorded in `renv.lock`. To restore the R
environment:

```r
install.packages("renv")
renv::restore()
```

The lockfile was generated with:

```text
R 4.6.0
Bioconductor 3.23
```

### Python Packages

```bash
pip install -r requirements.txt
```

## Example Workflow

Run the LUAD example pipeline:

```bash
bash run_luad_pipeline.sh
```

The script checks whether the expected LUAD input files already exist. If they
are present, the download step is skipped. Otherwise, TCGA data are downloaded
with `TCGAbiolinks`.

The run is logged to:

```text
data/logs/LUAD_pipeline_<timestamp>.log
```

Selected SVG figures generated by the pipeline are copied directly to:

```text
figures/
```

This directory is intended for GitHub upload, while the full `data/` directory
remains ignored.

## Individual Scripts

### 1. Download TCGA Data

```bash
Rscript src/01_download_tcga.R LUAD
```

Outputs:

```text
data/TCGA_LUAD/LUAD_rnaseq_counts.tsv
data/TCGA_LUAD/LUAD_rnaseq_log2tpm.tsv
data/TCGA_LUAD/LUAD_clinical.tsv
data/TCGA_LUAD/LUAD_snv.tsv
data/TCGA_LUAD/LUAD_cnv.tsv
```

### 2. Gene-Wise Survival Analysis

```bash
Rscript src/02_gene_survival_analysis.R LUAD continuous
```

Outputs:

```text
data/LUAD_coxph_continuous.tsv
```

### 3. Pathway Analysis

```bash
Rscript src/03_pathway_analysis.R data/LUAD_coxph_continuous.tsv
```

Outputs:

```text
data/LUAD_coxph_continuous_gsea_kegg.tsv
data/LUAD_coxph_continuous_gsea_reactome.tsv
data/LUAD_coxph_continuous_gsea_hallmark.tsv
data/LUAD_coxph_continuous_gsea_gobp.tsv
data/LUAD_coxph_continuous_gsea_gomf.tsv
data/LUAD_coxph_continuous_gsea_gocc.tsv
data/<analysis_file_basename>_ora_up_kegg.tsv
data/<analysis_file_basename>_ora_down_kegg.tsv
data/<analysis_file_basename>_ora_up_reactome.tsv
data/<analysis_file_basename>_ora_down_reactome.tsv
data/<analysis_file_basename>_ora_up_hallmark.tsv
data/<analysis_file_basename>_ora_down_hallmark.tsv
data/<analysis_file_basename>_ora_up_gobp.tsv
data/<analysis_file_basename>_ora_down_gobp.tsv
data/<analysis_file_basename>_ora_up_gomf.tsv
data/<analysis_file_basename>_ora_down_gomf.tsv
data/<analysis_file_basename>_ora_up_gocc.tsv
data/<analysis_file_basename>_ora_down_gocc.tsv
```

ORA uses the `HR` and `FDR` columns from the survival result. Significant genes
are selected using `FDR < 0.05`, then split into up (`HR > 1`) and down
(`HR < 1`) gene sets.

### 4. KEGG Metabolic Pathway Map

```bash
Rscript src/04_map_kegg_pathways.R data/LUAD_coxph_continuous_gsea_kegg.tsv
```

Output:

```text
data/LUAD_coxph_continuous_gsea_kegg_map.svg
```

![LUAD KEGG metabolic pathway map](figures/LUAD_coxph_continuous_gsea_kegg_map.svg)

### 5. ssGSEA Immune Signature Scoring

```bash
Rscript src/05_run_ssgsea.R LUAD immune_signature.gmt
```

Genes are retained for ssGSEA when they have at least 15 raw counts in at least
20% of retained primary tumor samples.

Outputs:

```text
data/LUAD_ssgsea_scores.tsv
data/LUAD_ssgsea_signature_correlation_heatmap.svg
data/LUAD_ssgsea_sample_score_heatmap.svg
```

| Signature correlation | Sample score heatmap |
| --- | --- |
| ![LUAD ssGSEA signature correlation heatmap](figures/LUAD_ssgsea_signature_correlation_heatmap.svg) | ![LUAD ssGSEA sample score heatmap](figures/LUAD_ssgsea_sample_score_heatmap.svg) |

### 6. mRNA Expression Analysis

Edit the user settings in `main()` or import the `AnalyzeExpression` class into
a notebook:

```bash
python src/06_analyze_expression.py
```

Outputs:

```text
data/LUAD_PLA2G4A_paired_tumor_normal_stats.tsv
data/LUAD_PLA2G4A_paired_tumor_normal_box.svg
data/LUAD_PLA2G4A_STK11_mutation_group_stats.tsv
data/LUAD_PLA2G4A_STK11_mutation_group_box.svg
data/LUAD_PLA2G4A_PTGS2_scatter_sample_type.svg
data/LUAD_PLA2G4A_PTGS2_STK11_scatter.svg
data/LUAD_STK11_gene_group_heatmap.svg
```

| Tumor-normal comparison | STK11 mutation groups |
| --- | --- |
| ![LUAD PLA2G4A tumor-normal comparison](figures/LUAD_PLA2G4A_paired_tumor_normal_box.svg) | ![LUAD PLA2G4A STK11 mutation group comparison](figures/LUAD_PLA2G4A_STK11_mutation_group_box.svg) |

| Sample-type scatter | STK11-status scatter |
| --- | --- |
| ![LUAD PLA2G4A PTGS2 scatter by sample type](figures/LUAD_PLA2G4A_PTGS2_scatter_sample_type.svg) | ![LUAD PLA2G4A PTGS2 scatter by STK11 status](figures/LUAD_PLA2G4A_PTGS2_STK11_scatter.svg) |

| STK11 gene-group expression heatmap |
| --- |
| ![LUAD STK11 gene-group expression heatmap](figures/LUAD_STK11_gene_group_heatmap.svg) |

### 7. Oncoprint

```bash
python src/07_oncoprint.py LUAD TP53 KRAS STK11 KEAP1 EGFR NF1
```

Outputs:

```text
data/LUAD_oncoprint_fisher.tsv
data/LUAD_oncoprint_fisher_heatmap.svg
data/LUAD_oncoprint.svg
```

| Oncoprint | Fisher test heatmap |
| --- | --- |
| ![LUAD oncoprint](figures/LUAD_oncoprint.svg) | ![LUAD oncoprint Fisher heatmap](figures/LUAD_oncoprint_fisher_heatmap.svg) |

## Notes

- TCGA data, GDC downloads, generated figures, intermediate R objects, and logs
  are excluded by `.gitignore`.
- `immune_signature.gmt` is used as an example GMT input for ssGSEA. Before
  publishing publicly, confirm that the file can be redistributed.
- Run scripts from the repository root so relative paths resolve correctly.
